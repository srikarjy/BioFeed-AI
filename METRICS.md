...
| Ranking model | **Two-tower + LightGBM reranker implemented and tested against synthetic interaction data** (`app/ml/two_tower.py`, `reranker.py`, `tests/test_recommendations.py`); **not trained on real usage** — `MODEL_DIR` has no checkpoint, so `GET /users/{id}/feed` (v0.7 route) correctly falls back to the v0.6 heuristic feed (`crud.get_personalized_feed`) at runtime | Train via `scripts/train_v07.py` once v0.3/v0.4 produce real interaction logs |
...
| CI | GitHub Actions: `alembic upgrade head` against a pgvector service + pytest (API, RSS, CRUD, dedup, retrieval, retrieval-eval, recommendations, anomaly) on Python 3.12; backend image built on PRs, published to GHCR on `main` | Extend to the anomaly/LLM path once GPU serving is verified |
