...
| LLM-generated explanations | **Fully verified on real GPUs, T4 and L4, direct and full-relay**: T4 direct TTFT p50 105ms; T4 full-relay TTFT p50 147ms; **L4 direct TTFT p50 57.3ms, L4 full-relay TTFT p50 89.1ms** (matched concurrency 10 — the originally targeted GPU class, isolating a real ~32ms FastAPI-hop cost). Full-relay path is the real thing end to end: `GET /internal/anomaly-explain/{id}` → `app/llm/client.py` → real vLLM → SSE, no stand-ins. See `benchmarks/report.md` for all four rows. | A from-scratch run with real Postgres in the loop (every GPU run so far used SQLite for setup speed) |
