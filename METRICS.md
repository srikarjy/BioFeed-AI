...
| Knowledge graph | **v0.8 shipped**: ~30 hand-curated entities grounded in real, live-verified ontology identifiers (MONDO/HPO for disease, ChEMBL for drug, HGNC for gene — substituted for UMLS, which needs a license/API key this project doesn't have), dictionary-matched extraction wired into every ingestion run, co-occurrence relations with type-informed predicates | Expand gazetteer coverage or add a trained NER model as a second pass; ground `trial` entities via ClinicalTrials.gov |
...
| CI | **Fixed this pass** — `backend-ci` had been failing on every push since the v0.6/v0.7 commit (2026-08-04) because `app.main` unconditionally imported torch/lightgbm, which the light `requirements.txt` CI installs doesn't include; confirmed green via `gh run list` after the fix, and re-confirmed after adding auth/kg/explain by building a clean venv from `requirements.txt` alone. GitHub Actions: `alembic upgrade head` against a pgvector service + pytest (67 tests: API, RSS, CRUD, dedup, retrieval, retrieval-eval, recommendations, auth, kg, explain, anomaly) on Python 3.12; backend image built on PRs, published to GHCR on `main` | Extend to the anomaly/LLM path once GPU serving is verified |
