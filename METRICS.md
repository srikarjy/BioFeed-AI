...
| Explainability | **v0.9 shipped**: `GET /users/{id}/articles/{id}/explain` returns a structured, multi-signal explanation (nearest-interaction similarity, topic/source affinity, freshness, popularity, source quality, shared KG entities), each with its own weight — not just the v0.6 feed's single heuristic string (`crud._generate_reason`, still used for the cheap list-view `reason` field) | Surface in a UI once v0.4 exists; replace the source-as-topic-affinity proxy with real per-article topics from KG entities |
| Knowledge graph | **v0.8 shipped, expanded 2026-08-16**: 49 hand-curated entities (disease/gene/company) grounded in real, live-verified ontology identifiers (MONDO/HPO, HGNC — substituted for UMLS, which needs a license/API key this project doesn't have), plus **dynamic trial-entity grounding** — NCT ids extracted via regex and looked up live against ClinicalTrials.gov per article (not a static list; cached locally after first sighting). Dictionary-matched extraction wired into every ingestion run, co-occurrence relations with type-informed predicates (now including company/drug/disease ↔ trial). New drug entries deferred — ChEMBL's API was returning HTTP 500 during this pass | Retry ChEMBL drug expansion once its API is stable; add a trained NER model as a second extraction pass to catch entities outside the gazetteer entirely |
