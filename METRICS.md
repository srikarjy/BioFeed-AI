...
| Metric | Current | Building toward |
|---|---|---|
| Users | **0 real users** — `User`/`UserInteraction`/`UserEmbedding` models, full CRUD, and now real JWT auth (v0.3) all exist and are tested, but nothing has signed up because there's no mobile app (v0.4) or registered Apple/Google app to sign in through yet | Real users via the iOS app once v0.4 ships |
| Auth | **v0.3 shipped, backend-only** — JWT access/refresh issuance, `require_self` enforced on every user-scoped route (401/403 tested), real Apple/Google JWKS verifiers written but unverified beyond code review (no registered app/real device to test against); `AUTH_PROVIDER=fake` is the default everywhere until real credentials exist | Verify real Apple/Google verification end to end once v0.4 exists |
...
| Explainability | **v0.9 shipped, topic-affinity fixed 2026-08-16**: `GET /users/{id}/articles/{id}/explain` returns a structured, multi-signal explanation (nearest-interaction similarity, topic/source affinity, freshness, popularity, source quality, shared KG entities), each with its own weight — not just the v0.6 feed's single heuristic string. `topic_affinity` is now keyed by real KG entity id (weighted by frequency across the user's positive interactions) instead of proxying article source — two articles from different sources mentioning the same company now correctly score affinity overlap, which the old source-proxy signal could never see | Surface in a UI once v0.4 exists |
| Knowledge graph | **v0.8 shipped, expanded twice 2026-08-16**: 54 hand-curated entities (disease/gene/company/drug) grounded in real, live-verified ontology identifiers (MONDO/HPO, HGNC, ChEMBL — substituted HGNC/HPO for UMLS, which needs a license/API key this project doesn't have), plus **dynamic trial-entity grounding** — NCT ids extracted via regex and looked up live against ClinicalTrials.gov per article (not a static list; cached locally after first sighting). Dictionary-matched extraction wired into every ingestion run, co-occurrence relations with type-informed predicates (company/drug/disease/gene ↔ trial included) | Add a trained NER model as a second extraction pass to catch entities outside the gazetteer entirely |
...
| LLM-generated explanations | **Fully verified on real GPUs, T4 and L4, SQLite and Postgres, direct and full-relay**: T4 direct TTFT p50 105ms; T4 full-relay (SQLite) TTFT p50 147ms; **L4 direct TTFT p50 57.3ms, L4 full-relay TTFT p50 89.1ms** (matched concurrency 10, isolating a real ~32ms FastAPI-hop cost); **T4 full-relay on real Postgres (built from source, real `alembic upgrade head`) TTFT p50 186ms** — a real, now-quantified Postgres-vs-SQLite overhead. Full-relay path is the real thing end to end: `GET /internal/anomaly-explain/{id}` → `app/llm/client.py` → real vLLM → SSE, no stand-ins. See `benchmarks/report.md` for all five rows. | A10G/H-class hardware beyond T4/L4; a live Grafana screenshot |
...
| Anomaly-explain, real vLLM on GPU (`vllm-t4-direct`) | **Measured 2026-08-15** on a Hugging Face Jobs T4: concurrency 10 → 1.8 req/s, 254 tokens/s, TTFT p50 105ms/p95 2.1s, total-latency p50 4.9s; concurrency 30 → 2.7 req/s, 385 tokens/s, TTFT p50 5.2s, total-latency p50 10.5s. Hits vLLM directly. Job cost ~$0.06 (525s). | — |
| Anomaly-explain, full relay on GPU (`vllm-t4-full-relay`) | **Measured 2026-08-16** — real FastAPI backend + real vLLM in the same Hugging Face Jobs T4 container, real route → real SSE relay, no stand-ins: concurrency 5 → 1.4 req/s, 152 tokens/s, TTFT p50 147ms/p95 2.2s. Required a real fix first — SQLite needs `check_same_thread=False`+`StaticPool` for a real `uvicorn` process to not crash (`app/database.py`). Job cost ~$0.04 (378s). | — |
| Anomaly-explain, matched-concurrency L4 comparison (`vllm-l4-direct` / `vllm-l4-full-relay`) | **Measured 2026-08-16** on a Hugging Face Jobs `l4x1` (1x L4, 24GB — the GPU class `llm_serving/serve.sh` actually targets), both direct and full-relay in one job at concurrency 10: direct TTFT p50 57.3ms/p95 408.7ms; full-relay TTFT p50 89.1ms/p95 377.3ms — a real, isolated ~32ms FastAPI-hop cost. Vs. T4 at the same concurrency: TTFT p50 roughly halved (104.8ms→57.3ms), p95 tail cut ~5x (2104ms→409ms). Job cost ~$0.09 (389s at $0.80/hr). See `benchmarks/report.md`. | — |
| Anomaly-explain, real Postgres from scratch (`vllm-t4-postgres-full-relay`) | **Measured 2026-08-16** — PostgreSQL 16 + pgvector built from source in a Hugging Face Jobs T4 container, real `alembic upgrade head` (migrations 0001–0006, the same chain CI runs), real ingestion → `embed_missing` → KG extraction → anomaly detection pipeline, real pgvector HNSW `GET /search` (cosine similarity 0.3814 on the seeded articles, confirmed separately), then the full relay at concurrency 10: 30/30 ok, TTFT p50 186ms/p95 2531ms, 246 tokens/s. Job cost ~$0.05 (412s). See `benchmarks/report.md`. | — |
...
| CI | **Fixed this pass** — `backend-ci` had been failing on every push since the v0.6/v0.7 commit (2026-08-04) because `app.main` unconditionally imported torch/lightgbm, which the light `requirements.txt` CI installs doesn't include; confirmed green via `gh run list` after the fix, and re-confirmed after adding auth/kg/explain/trials by building a clean venv from `requirements.txt` alone. GitHub Actions: `alembic upgrade head` against a pgvector service + pytest (77 tests: API, RSS, CRUD, dedup, retrieval, retrieval-eval, recommendations, auth, kg incl. trial grounding, explain, signals, anomaly) on Python 3.12; backend image built on PRs, published to GHCR on `main` | Extend to the anomaly/LLM path once GPU serving is verified |
| Monitoring | `IngestionRun` history for the core pipeline; Prometheus + Grafana stood up for the LLM path (`observability/`), scraping vLLM's native `/metrics`. **Stale metric names found and fixed 2026-08-16**: `gpu_cache_usage_perc`→`kv_cache_usage_perc` (renamed), `cpu_cache_usage_perc`→prefix-cache hit rate (no direct replacement exists — this vLLM version dropped CPU/swap cache tracking), `time_per_output_token_seconds`→`inter_token_latency_seconds` (renamed) — all confirmed present in real `/metrics` output from a live vLLM server. Still not screenshotted against a live Grafana instance — a metric-name diff isn't a rendered dashboard | Verify the dashboard actually renders against a live Prometheus scrape; extend Prometheus/Grafana to core ingestion/API |
| Database | PostgreSQL 16 (**pgvector image**), SQLAlchemy 2.0 typed ORM, Alembic migrations 0001–0006 (articles → dedup/runs → embeddings → anomaly → recommendation engine → knowledge graph) | Decision made: pgvector in-database, no FAISS sidecar until latency demands it |
