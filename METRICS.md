...
| LLM-generated explanations | **Fully verified, twice, on real GPUs**: (1) `TheBloke/Mistral-7B-Instruct-v0.2-AWQ` served via `vllm/vllm-openai` on a Hugging Face Jobs T4, hit directly — TTFT p50 105ms at concurrency 10; (2) the real product path — real FastAPI backend + real vLLM in the same GPU job, `GET /internal/anomaly-explain/{id}` → `app/llm/client.py` → vLLM → SSE, no stand-ins — TTFT p50 147ms, 15/15 requests. See `benchmarks/report.md` for both. | A10G/L4-class numbers (the originally targeted GPU class) vs. this T4 baseline; matched-concurrency comparison between the direct and full-relay runs |
...
| Anomaly-explain, real vLLM on GPU (`vllm-t4-direct`) | **Measured 2026-08-15** on a Hugging Face Jobs T4 (`hf jobs run --flavor t4-small`, `TheBloke/Mistral-7B-Instruct-v0.2-AWQ`): concurrency 10 → 1.8 req/s, 254 tokens/s, TTFT p50 105ms/p95 2.1s, total-latency p50 4.9s; concurrency 30 → 2.7 req/s, 385 tokens/s, TTFT p50 5.2s, total-latency p50 10.5s. Hits vLLM directly, not through the FastAPI SSE relay. Job cost ~$0.06 (525s). | A10G/L4-class numbers for comparison against this T4 baseline |
| Anomaly-explain, full relay on GPU (`vllm-t4-full-relay`) | **Measured 2026-08-16** — real FastAPI backend + real vLLM in the same Hugging Face Jobs T4 container, real route (`GET /internal/anomaly-explain/{id}`) → real SSE relay, no stand-ins: concurrency 5 → 1.4 req/s, 152 tokens/s, TTFT p50 147ms/p95 2.2s, total-latency p50 2.7s. Required a real fix first — SQLite needs `check_same_thread=False`+`StaticPool` for a real `uvicorn` process to not crash (`app/database.py`, production always used Postgres so this was unnoticed). Job cost ~$0.04 (378s). See `benchmarks/report.md`. | Matched-concurrency comparison against `vllm-t4-direct`; A10G/L4 numbers; a from-scratch run with real Postgres (this used SQLite for GPU-box setup speed) |
...
| Monitoring | `IngestionRun` history for the core pipeline; Prometheus + Grafana stood up for the LLM path (`observability/`), scraping vLLM's native `/metrics`. **Metric names checked against real output 2026-08-16**: 7/10 dashboard panel queries match live vLLM `/metrics`; 3 don't (`gpu_cache_usage_perc`, `cpu_cache_usage_perc`, `time_per_output_token_seconds` — likely renamed in a newer vLLM release). Still not screenshotted against a live Grafana instance — a metric-name diff isn't a rendered dashboard | Fix the 3 stale metric names, then verify the dashboard actually renders against a live Prometheus scrape; extend Prometheus/Grafana to core ingestion/API |
